...
if (Test-Path $RsvarsPath) {
    # Compilar Delphi
    $CmdLine = "call `"$RsvarsPath`" && dcc32 -NS`"System;Xml;Data;Web;Soap;Winapi;System.Win`" -U`"$BdsPath\lib\Win32\Release`" -U`"$ProjectRoot`" -U`"$ProjectRoot\modules\horse\src`" -E`"$DelphiBin`" -N`"$DelphiBin`" `"$TestsDir\Horse.OpenTelemetry.Tests.dpr`""
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        $DelphiExe = Join-Path $DelphiBin "Horse.OpenTelemetry.Tests.exe"
        & $DelphiExe
...
if (Test-Path $FpcPath) {
    ...
    # Compilar FPC
    & $FpcPath -Mdelphi -FE"$LazarusBin" -FU"$LazarusBin" -Fu"$ProjectRoot" -Fu"$ProjectRoot\modules\horse\src" "$TestsDir\Horse.OpenTelemetry.Tests.dpr"

    if ($LASTEXITCODE -eq 0) {
        Write-Host "Compilação concluída com sucesso! Executando testes..." -ForegroundColor Green
        $FpcExe = Join-Path $LazarusBin "Horse.OpenTelemetry.Tests.exe"
        & $FpcExe
        if ($LASTEXITCODE -ne 0) {
